...
class SentenceRE(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.typeset_size = hparams.typeset_size
        self.use_entity_type = hparams.use_entity_type
        ...
        self.norm = nn.LayerNorm(self.embedding_dim * 3 + self.typeset_size * 2)
        self.hidden2tag = nn.Linear(self.embedding_dim * 3 + self.typeset_size * 2, self.tagset_size)

    def forward(self, token_ids, token_type_ids, attention_mask, e1_mask, e2_mask, type_mask=None):
        ...
        if not self.use_entity_type:
            # [cls] + 实体1 + 实体2
            concat_h = torch.cat([pooled_output, e1_h, e2_h], dim=-1)
        else:
            type_h = self.activation(type_mask.float())
            concat_h = torch.cat([pooled_output, e1_h, e2_h, type_h], dim=-1)
