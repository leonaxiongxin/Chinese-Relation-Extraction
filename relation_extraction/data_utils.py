...
class MyTokenizer(object):
    def __init__(self, typeset_path=None, pretrained_model_path=None, mask_entity=False):
        ...
        self.type2idx = get_type2idx(typeset_path)

    def tokenize(self, item):
        sentence = item['text']
        pos_head = item['h']['pos']
        pos_tail = item['t']['pos']
        type_head = item['h']['etype']
        type_tail = item['t']['etype']
        ...
        pos1 = [0, 0]
        pos2 = [0, 0]
        type1 = self.type2idx[type_head]
        type2 = self.type2idx[type_tail]
        for token in tokens:
            token = token.lower()
        ...
        return re_tokens[1:-1], pos1, pos2, type1, type2
...
def convert_type_to_mask(e1_type_idx, e2_type_idx, typeset_file):
    max_type = len(get_type2idx(typeset_file))
    type_mask = [0] * (max_type * 2)
    type_mask[e1_type_idx] = 1
    type_mask[e2_type_idx + max_type] = 1
    return type_mask


def read_data(input_file, typeset_file, tokenizer=None, max_len=128):
    tokens_list = []
    e1_mask_list = []
    e2_mask_list = []
    tags = []
    ...
    with open(input_file, 'r', encoding='utf-8') as f_in:
        for line in tqdm(f_in):
            ...
            tokens, pos_e1, pos_e2, type_e1, type_e2 = tokenizer.tokenize(item)
            if pos_e1[0] < max_len - 1 and pos_e1[1] < max_len and \
                ...
                tag = item['relation']
                tags.append(tag)
                type_mask = convert_type_to_mask(type_e1, type_e2, typeset_file)
                type_mask_list.append(type_mask)
    return tokens_list, e1_mask_list, e2_mask_list, tags, type_mask_list
...
def get_type2idx(file):
    with open(file, 'r', encoding='utf-8') as f_in:
        typeset = re.split(r'\s+', f_in.read().strip())
    return dict((etype, idx) for idx, etype in enumerate(typeset))
...
class SentenceREDataset(Dataset):
    def __init__(self, data_file_path, tagset_path, typeset_path, pretrained_model_path=None, max_len=128):
        self.data_file_path = data_file_path
        self.tagset_path = tagset_path
        self.typeset_path = typeset_path
        self.pretrained_model_path = pretrained_model_path or 'bert-base-chinese'
        self.tokenizer = MyTokenizer(typeset_path=self.typeset_path, pretrained_model_path=self.pretrained_model_path)
        self.max_len = max_len
        self.tokens_list, self.e1_mask_list, self.e2_mask_list, self.tags, self.type_mask_list = read_data(data_file_path, self.typeset_path, tokenizer=self.tokenizer, max_len=self.max_len)
    ...
    def __getitem__(self, idx):
        ...
        sample_type_mask = self.type_mask_list[idx]
        ...
        sample_tag_id = self.tag2idx[sample_tag] if sample_tag in self.tag2idx else self.tag2idx["NA"]
        ...
        sample = {
            ...
            'type_mask': torch.tensor(sample_type_mask)
        }
        return sample
