...
from .data_utils import SentenceREDataset, get_idx2tag, get_type2idx, load_checkpoint, save_checkpoint
...
def train(hparams):
    ...
    # train_dataset
    train_dataset = SentenceREDataset(train_file, tagset_path=tagset_file, typeset_path=typeset_file,
    ...
    for epoch in range(epoch_offset, epochs):
        ...
        for i_batch, sample_batched in enumerate(tqdm(train_loader, desc='Training')):
            ...
            type_mask = sample_batched['type_mask'].to(device)
            model.zero_grad()
        ...
        if validation_file:
            validation_dataset = SentenceREDataset(validation_file, tagset_path=tagset_file, typeset_path=typeset_file,
                                                   pretrained_model_path=pretrained_model_path,
                                                   max_len=max_len)
